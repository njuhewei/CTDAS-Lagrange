...
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/rsource/load.ncdf4.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/rsource/id2info.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/rsource/julian.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/rsource/time.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/rsource/ddate.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/rsource/datelist.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/stilt-cvs-20090417/stiltR/sourceall.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/stilt-cvs-20090417/stiltR/Trajecfoot.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/stilt-cvs-20090417/stiltR/getgridp.r")
source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/rsource/assignr.r")

source("/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/Rcode/rsource/output.ncdf4.v2.3.r")
...
curdir="/Storage/CO2/wei/test_additive_fluxfac/ctdas-stilt-base/exec/da/stilt/"
...
# flags of prior flux and BC data
...
ncol2=120                         #number of pixels in x directions in grid 
nrow2=70                          #number of pixels in y directions in grid 
LLLon2=-169.5                     #lower left corner of grid 
LLLat2=10.5                       #lower left corner of grid 
...
for(i in 0:(nsam-1))   
{
  ...
  if (b1>=b0) 
  {
    ...
    scalefacarr1[,,i+1] = scalefac * 1e6   #convert to umol/m2/s
  ...
  scalefac <- ncvar_get(ncf,"parametermap",start=c(11,101),count=c(ncol2,nrow2))  
  scalefacarr2[,,i+1] = scalefac * 1e6    #convert to umol/m2/s
  scalefac <- ncvar_get(ncf,"parametervalues_bc")
...
for(yy in 1:length(uyr))
{
  ...
  {
      pfbpath=paste(footpdir,uyr[yy],"/",umon[mm],"/",sep="") 
...
for(mm in 1:length(pfbfns))  
{
  ...
  #flog.info('eventids: %s',eventids[mm], name='logger.b') 
  ...
  {			
    ...
    colnames(endpts)=endptna        # "time","index","lat","lon","agl","grdht","temp","pres" 
   ...
   #----------------------------------------------------
    ...
    firflux[,,]=replace(firflux[,,],is.na(firflux[,,]),0)
     ...
     if(bioflux_flag == "SiBCASA")
         neeflux[,,]=(recflux[,,]-gppflux[,,])*1e6  #convert to umol/m2/s
    ...
    for (i in 1:nsam) 
    {
      ...
      {    
        ...
        diff=ceiling(diff)
        align=24*ceiling((ntimes-diff)/24) 
...
{
    nmem=(dim(data)[2]-8)/3  
    vals=data[,2:(3*nsam+8)]
}

if(bioflux_flag == "CT_OPT")
{
    nmem=(dim(data)[2]-7)/3  
    vals=data[,2:(3*nsam+7)] 
}
# use a flag to specify for different flux simulation (SiB3 or CT_OPT)
write.results.netcdf(bioflux_flag,vals,nobs,nmem,fout)  
